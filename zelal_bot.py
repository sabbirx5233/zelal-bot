...
# --- FLASK KEEP-ALIVE SERVER SETUP FOR RENDER ---
app = Flask(__name__)
...
ADMIN_ID = 6878235212
DEFAULT_WATERMARK_TEXT = "t.me/ldrzelalx"
...
async def send_force_join_msg(update: Update):
    ...
    msg_text = "⚠️ *বটটি ব্যবহার করতে আপনাকে প্রথমে আমাদের চ্যানেলে জয়েন করতে হবে!*\n\nনিচের লিংকে গিয়ে জয়েন করে **Verify / Check Join** বাটনে ক্লিক করুন।"
    if update.message:
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    welcome_text = (
        ...
        "নিচের স্থায়ী বাটনগুলো থেকে আপনার প্রয়োজনীয় সার্ভিসটি বেছে নিন:"
    )
    await update.message.reply_text(welcome_text, parse_mode="Markdown", reply_markup=MAIN_KEYBOARD)

async def ask_watermark_option(update: Update, mode_name: str):
    keyboard = InlineKeyboardMarkup([
        [InlineKeyboardButton(f"🏷️ Default ({DEFAULT_WATERMARK_TEXT})", callback_data="wm_default")],
        [InlineKeyboardButton("✏️ Custom Text (নিজের নাম/লিংক)", callback_data="wm_custom")]
    ])
    await update.message.reply_text(
        f"✍️ *{mode_name}-এর জন্য ওয়াটারমার্ক সিলেক্ট করুন:*\n\nআপনি কি ডিফল্ট টেক্সট নাকি নিজের কোনো লেখা যোগ করতে চান?",
        parse_mode="Markdown",
    ...
    if query.data == "check_join":
        if await is_user_subscribed(context.bot, user_id):
            ...
            await context.bot.send_message(
                chat_id=user_id, 
                text="✅ *ভেরিফিকেশন সফল হয়েছে!* এখন নিচের বাটন ব্যবহার করে বটটি উপভোগ করুন।", 
                parse_mode="Markdown", 
                reply_markup=MAIN_KEYBOARD
            )
        else:
            await query.message.reply_text("❌ *আপনি এখনো চ্যানেলে জয়েন করেননি!* জয়েন করে আবার চেষ্টা করুন।")

    elif query.data == "wm_default":
        USER_WATERMARKS[user_id] = DEFAULT_WATERMARK_TEXT
        state = USER_STATES.get(user_id)
        await query.message.delete()
        
        if state == "IMAGE":
            await context.bot.send_message(chat_id=user_id, text=f"✅ ওয়াটারমার্ক সেট করা হয়েছে: `{DEFAULT_WATERMARK_TEXT}`\n\nএখন একটি **Photo** পাঠান।", parse_mode="Markdown")
        elif state == "VIDEO":
            await context.bot.send_message(chat_id=user_id, text=f"✅ ওয়াটারমার্ক সেট করা হয়েছে: `{DEFAULT_WATERMARK_TEXT}`\n\nএখন একটি **Video** পাঠান।", parse_mode="Markdown")

    elif query.data == "wm_custom":
        USER_STATES[user_id] = "WAITING_WM_TEXT_" + USER_STATES.get(user_id, "")
        await query.message.delete()
        await context.bot.send_message(chat_id=user_id, text="✏️ *আপনার কাস্টম ওয়াটারমার্ক টেক্সটটি লিখে পাঠান:*", parse_mode="Markdown")
...
async def download_social_video(update: Update, url: str):
    ...
    try:
        ...
        if os.path.exists(filename):
            ...
            await msg.edit_text("❌ ভিডিওটি ডাউনলোড করা সম্ভব হয়নি!")
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Handle custom watermark text entry
    current_state = USER_STATES.get(user_id, "")
    if current_state.startswith("WAITING_WM_TEXT_"):
        original_mode = current_state.replace("WAITING_WM_TEXT_", "")
        USER_WATERMARKS[user_id] = text
        USER_STATES[user_id] = original_mode
        
        if original_mode == "IMAGE":
            await update.message.reply_text(f"✅ কাস্টম ওয়াটারমার্ক সেট করা হয়েছে: `{text}`\n\nএখন একটি **Photo** পাঠান।", parse_mode="Markdown")
        elif original_mode == "VIDEO":
            await update.message.reply_text(f"✅ কাস্টম ওয়াটারমার্ক সেট করা হয়েছে: `{text}`\n\nএখন একটি **Video** পাঠান।", parse_mode="Markdown")
        return
    ...
    if text == "📸 Image to Motion Video":
        ...
        await ask_watermark_option(update, "Image to Motion")
    elif text == "🎬 Video Watermark & Sound":
        USER_STATES[user_id] = "VIDEO"
        await ask_watermark_option(update, "Video Watermark")
...
async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    state = USER_STATES.get(user_id)
    wm_text = USER_WATERMARKS.get(user_id, DEFAULT_WATERMARK_TEXT)
    ...
    if state == "IMAGE":
        ...
        try:
            ...
            draw.text((text_x, text_y), wm_text, fill=(255, 255, 255, 255), font=font)
            ...
            with open(output_video, 'rb') as video:
                await context.bot.send_video(chat_id=update.effective_chat.id, video=video, caption=f"🎬 *MOTION VIDEO GENERATED*\n🏷️ *Watermark:* `{wm_text}`", parse_mode="Markdown", reply_markup=MAIN_KEYBOARD)
            await msg.delete()
        except Exception as e:
...
async def handle_video(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not state:
        await update.message.reply_text("⚠️ দয়া করে নিচের স্থায়ী কিবোর্ড থেকে একটি অপশন সিলেক্ট করুন!", reply_markup=MAIN_KEYBOARD)
        return
    ...
    try:
        ...
        if state == "EXTRACT_AUDIO":
            ...
            if extract_audio_mp3(input_video, output_audio):
                ...
                await msg.edit_text("❌ এই ভিডিওতে কোনো সাউন্ড ট্র্যাক পাওয়া যায়নি!")
        ...
        # 2. ADD WATERMARK & SOUND
            ...
            (text_w, text_h), baseline = cv2.getTextSize(wm_text, cv2.FONT_HERSHEY_SIMPLEX, font_scale, thickness)
            ...
            while cap.isOpened():
                ...
                cv2.putText(frame, wm_text, (text_x, text_y), cv2.FONT_HERSHEY_SIMPLEX, font_scale, (0, 0, 0), thickness + 3, cv2.LINE_AA)
                cv2.putText(frame, wm_text, (text_x, text_y), cv2.FONT_HERSHEY_SIMPLEX, font_scale, (255, 255, 255), thickness, cv2.LINE_AA)
                out.write(frame)
            ...
            with open(send_path, 'rb') as video:
                await context.bot.send_video(chat_id=update.effective_chat.id, video=video, caption=f"✨ *PROCESSED HD VIDEO*\n🏷️ *Watermark:* `{wm_text}`", parse_mode="Markdown", reply_markup=MAIN_KEYBOARD)
            await msg.delete()
        ...
        # 3. SPEED CONTROLLER (2X FAST)
            ...
            if os.path.exists(speed_video):
                ...
                await msg.edit_text("❌ স্পিড পরিবর্তন করার সময় সমস্যা হয়েছে!")

        # 4. FIXED VIDEO WATERMARK REMOVER
            ...
            if os.path.exists(clean_vid) and os.path.getsize(clean_vid) > 0:
                ...
                if os.path.exists(clean_vid):
                    ...
                    await msg.edit_text("❌ ওয়াটারমার্ক রিমুভ করতে সমস্যা হয়েছে!")
...
def main():
    ...
    print("🚀 Bot is Running with Custom Watermark & Keep-Alive...")
    app.run_polling()
